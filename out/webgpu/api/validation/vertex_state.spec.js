...
desc(
`Test that the vertex buffer arrayStride must be less than <maxVertexBufferArrayStride>
...
desc(
`Test that the vertex buffer arrayStride must be aligned to 4
   - Test for various vertex buffer indices
...
`Test that the vertex buffer arrayStride must be aligned to 4
   ...
   - Test for shaderLocation 0, 1, limit -1, limit (limit is maxVertexAttributes)`).

subcases(() =>
params().
combine(poptions('vertexBufferIndex', [0, 1, MAX_VERTEX_BUFFERS - 1])).
combine(poptions('attributeCount', [0, 1, MAX_VERTEX_ATTRIBUTES - 1])).
...
g.test('check_out_of_bounds_condition_on_attribute_shader_location').fn(async t => {
  const vertexState = {
    vertexBuffers: [
    {
      arrayStride: 0,
      attributes: [
      { format: 'float', offset: 0, shaderLocation: MAX_VERTEX_ATTRIBUTES - 1 }] }] };
...
g.test('check_multiple_of_4_bytes_constraint_on_offset').fn(async t => {
  const vertexState = {
    vertexBuffers: [
    {
      arrayStride: 0,
      attributes: [
      { format: 'float', offset: SIZEOF_FLOAT, shaderLocation: 0 }] }] };
